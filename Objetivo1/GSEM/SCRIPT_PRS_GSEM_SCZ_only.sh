...
# ==============================================
# Description -
#   El script utiliza los datos de genotipado de una muestra objetivo (cohorte de casos - controles EUGEI) que se quiere analizar (dataTarget) y los summary statistics de GWAS de esquizofrenia obtenido de Genomic SEM (dataDiscovery).
#
# ==============================================
...
# summary statistics (SCZ_only)
setwd("/home/nodotea/PATRICIA/Nuevos_objetivos/Objetivo1/Discovery")
dataDiscovery <- read.table('SCZ_only_GSEM.txt', header=TRUE, stringsAsFactors = FALSE)  ## 1,150,181  variantes
...
### este paso no se hace en SCZ_only
...
SCZ_only_SCORE <- dataDiscovery_filtered[,c("SNP", "A1", "BETA")]

SCZ_only_P <- dataDiscovery_filtered[,c("SNP", "P")]
write.table(SCZ_only_SCORE, "SCZ_only_SCORE.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
write.table(SCZ_only_P, "SCZ_only_P.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
...
setwd("/home/nodotea/PATRICIA/Nuevos_objetivos/Objetivo1/Calculos_PRS")
system2("./plink", args=c("--bfile EUGEI_WP6_Case_control_noMHC_filtered_CLUMPED", "--score SCZ_only_SCORE.txt", "--q-score-file SCZ_only_P.txt", "--q-score-range Q_RANGES.txt", "--out SCZ_only_PRS_SCORE"))
...
write.table(covars, "covariables_SCZ_only.txt", row.names = FALSE, col.names = TRUE, dec = ".")

covars <- read.table("covariables_SCZ_only.txt", header = TRUE, stringsAsFactors = FALSE)
covars_filter <- covars[covars$dcode %in% c(1, 2),] # Los pheno -9 se quitan. 
variables_SCZ_only <- covars_filter

for (j in 1:12)  # Pongo 12 porque he generado 12 PRS para los 12 umbrales del archivo Q_RANGES
	{
  	fin <- read.table(paste0("SCZ_only_PRS_SCORE.S",j,".profile"), header = TRUE, stringsAsFactors =  FALSE)
  	score <- data.frame(FID= fin$FID, IID= fin$IID, fin$SCORE)
  	colnames(score)[3] <- paste0("SCORE_S",j)
  	variables_SCZ_only <- merge(variables_SCZ_only, score, by = c("IID", "FID"))
	}

write.table(variables_SCZ_only, "SCZ_only.txt", dec= ".", row.names = FALSE)  #### GUARDAR A RESULTADOS ####
...
library(rms)
library(pROC)
variables_SCZ_only <- read.table('SCZ_only.txt', header=TRUE, stringsAsFactors = FALSE)  

# Definimos variables de regresión logística. Metemos las 10 primeras PCs, sexo y edad, además de PRS.
pheno <- as.numeric(variables_SCZ_only$dcode)
PC1  <- variables_SCZ_only$C1
PC2  <- variables_SCZ_only$C2
PC3  <- variables_SCZ_only$C3
PC4  <- variables_SCZ_only$C4
PC5  <- variables_SCZ_only$C5
PC6  <- variables_SCZ_only$C6
PC7  <- variables_SCZ_only$C7
PC8  <- variables_SCZ_only$C8
PC9  <- variables_SCZ_only$C9
PC10 <- variables_SCZ_only$C10
SEX   <- variables_SCZ_only$scode
AGE   <- variables_SCZ_only$AGE
COUNTRY   <- variables_SCZ_only$COUNTRY
...
for (j in 1:12) 
	{
  	score <- variables_SCZ_only[,grep(paste0("^SCORE_S",j,"$"), colnames(variables_SCZ_only))]  # tenemos que ponerle esta sintaxis para que coja score1 y no score10,11,12 a la vez
...
data_SCZ_only_lrm = dataRanges # Mejor umbral S9 P < 0.1
...
rocobj <- plot.roc(variables_SCZ_only$dcode, variables_SCZ_only$SCORE_S9,
...
data_SCZ_only_lrm$R2_liab = (h2l_R2N(k, ((data_SCZ_only_lrm$R2)*0.01), p))*100
write.table(data_SCZ_only_lrm, "data_SCZ_only_lrm.txt", dec= ",", row.names = FALSE)
...
## Leemos archivo
table <- read.table("SCZ_only.txt", header=T, stringsAsFactors = FALSE)
...
data_SCZ_only_lrm_perm = dataRanges


# Añadimos el valor en liability scale
data_SCZ_only_lrm_perm$R2_liab = (h2l_R2N(k, ((data_SCZ_only_lrm_perm$R2)*0.01), p))*100
write.table(data_SCZ_only_lrm_perm, "data_SCZ_only_lrm_perm.txt", dec= ",", row.names = FALSE)



### Creamos una tabla donde aparecen los resultados en promedio, con los intervalos de confianza

table <- read.table("data_SCZ_only_lrm_perm.txt", header=T, stringsAsFactors = FALSE, dec = ",")
...
write.table(datos_nuevos, "data_SCZ_only_lrm_perm_CIs.txt", dec= ",", row.names = FALSE)


# Le añadimos el valor de P de la comparación inicial mediante glm si bootstrap.
table2 <- read.table("data_SCZ_only_lrm.txt", header=T, stringsAsFactors = FALSE, dec = ",")
...
write.table(TABLA_FINAL, "FINAL_data_SCZ_only_lrm_perm_CIs_P.txt", dec= ",", row.names = FALSE)
...
## Después hacemos una comparación de la probabilidad de ser un caso con SCZ_only dentro de cada decilr respecto al medio (5º) o respecto al decil más bajo (1º)
...
SCZ_only <- read.table('SCZ_only.txt', header=TRUE, stringsAsFactors = FALSE)  
SCZ_only$decile <- ntile(SCZ_only$SCORE_S4, 5) ### Asignar el umbral más significativo
...
for(i in y)
	{
  	variables_SCZ_only <- SCZ_only[ which(SCZ_only$decile == 5 | SCZ_only$decile == i), ]
 	write.table(variables_SCZ_only, paste0("data",i), dec= ",", row.names = FALSE)
	}
...
for(i in y)
	{
  	variables_SCZ_only <- read.table(paste0("data",i), header = TRUE, stringsAsFactors = FALSE, dec = ",")
  	variables_SCZ_only$comparison <- ifelse(variables_SCZ_only$decile == 5, 0, 1)
	...
	AGE   <- variables_SCZ_only$AGE
	COUNTRY <- variables_SCZ_only$COUNTRY
 	SCORE <- variables_SCZ_only$comparison
...
write.table(data_decils_lgm, "data_SCZ_only_lrm_DECIL_VS_5th.txt", dec= ",", row.names = FALSE)
...
for(i in y)
	{
  	variables_SCZ_only <- SCZ_only[ which(SCZ_only$decile == 1 | SCZ_only$decile == i), ]
 	write.table(variables_SCZ_only, paste0("data",i), dec= ",", row.names = FALSE)
	}
...
for(i in y)
	{
  	variables_SCZ_only <- read.table(paste0("data",i), header = TRUE, stringsAsFactors = FALSE, dec = ",")
  	variables_SCZ_only$comparison <- ifelse(variables_SCZ_only$decile == 1, 0, 1)
...
data_decils_lgm = dataRanges
write.table(data_decils_lgm, "data_SCZ_only_lrm_DECIL_VS_1st.txt", dec= ",", row.names = FALSE)
...
prs.result <- read.table("FINAL_data_SCZ_only_lrm_perm_CIs_P.txt", header=T, stringsAsFactors = FALSE, dec = ",")
...
table2=read.table("data_SCZ_only_lrm_DECIL_VS_5th.txt", header = T, dec = ",", stringsAsFactors = FALSE)
...
table2=read.table("data_SCZ_only_lrm_DECIL_VS_1st.txt", header = T, dec = ",", stringsAsFactors = FALSE)
table2$Decile <- sub("Decile_", "", table2$Decile)
table2$Decile <- factor(table2$Decile, levels = table2$Decile)
