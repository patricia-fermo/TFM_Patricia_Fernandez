...
# ==============================================
# Description -
#   El script utiliza los datos de genotipado de una muestra objetivo (cohorte de casos - controles EUGEI) que se quiere analizar (dataTarget) y los summary statistics de GWAS de aislamiento (dataDiscovery).
#
# ==============================================
...
# summary statistics (ISO)
dataDiscovery <- read.table('1031.gwas.imputed_v3.both_sexes.tsv', header=TRUE, stringsAsFactors = FALSE)  ## 7583883 variantes

setwd("/home/nodotea/PATRICIA/Objetivos/Datos/Target/WP2")
# Cohorte EUGEI WP2 (target sample)
dataTarget <- read.table('EUGEI_WP2_Case_control_noMHC.bim', header = FALSE, stringsAsFactors = FALSE) ## 8220796 variantes
...
library(tidyr)
# separar columna variant con regex
temp_discovery1<-separate(temp_discovery1, "variant", into =  c("CHR", "BP", "REF", "ALT"), sep = ":", extra ="drop")
...
temp_discovery2<-subset(temp_discovery1, !(C %in% c("AT","TA", "CG", "GC"))) ## 11862242 variantes
...
temp_discovery2_UNIQ<-temp_discovery2[temp_discovery2$POS %in% FRECUENCIAS_discovery$Var1[FRECUENCIAS_discovery$minor_AF < 2], ]
...
temp_discovery3<-dataDiscovery[,c("CHR", "BP", "REF", "ALT", "beta", "se", "pval", "POS")]
colnames(temp_discovery3)<-c("CHR", "BP", "A1", "A2", "BETA", "SE", "P", "SNP")
...
dataTarget_filtered <- dataTarget[which(dataTarget$varID %in% dataDiscovery$SNP),]                 ## 5404999       variantes
dataDiscovery_filtered <- dataDiscovery[which(dataDiscovery$SNP %in% dataTarget_filtered$varID),]  ## 5404999       variantes

#borramos duplicados
dataDiscovery_filtered <- dataDiscovery_filtered[!duplicated(dataDiscovery_filtered$SNP), ]
...
setwd("/home/nodotea/PATRICIA/Objetivos/Calculos_PRS")
system2("./plink", args=c("--bfile /home/nodotea/PATRICIA/Objetivos/Datos/Target/WP2/EUGEI_WP2_Case_control_noMHC", "--extract variableIDs_forFiltering.txt", "--exclude range MHC_filter.txt", "--make-bed", "--out EUGEI_WP2_Case_control_noMHC_new_filtered"))
...
system2("./plink", args=c("--bfile EUGEI_WP2_Case_control_noMHC_new_filtered", "--clump discovery_paraCLUMP.txt", "--clump-p1 1", "--clump-p2 1", "--clump-kb 500", "--clump-r2 0.1", "--out EUGEI_WP2_Case_control_noMHC_filtered_CLUMPED"))
CLUMPED_PLINK<-read.table("EUGEI_WP2_Case_control_noMHC_filtered_CLUMPED.clumped", header=T)
write.table(CLUMPED_PLINK$SNP, "CLUMPED_PLINK.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
system2("./plink", args=c("--bfile EUGEI_WP2_Case_control_noMHC_new_filtered", "--extract CLUMPED_PLINK.txt", "--make-bed", "--out EUGEI_WP2_Case_control_noMHC_filtered_CLUMPED"))  ## 210691 variantes 
...
ISO_SCORE <- dataDiscovery_filtered[,c("SNP", "A1", "BETA")]



ISO_P <- dataDiscovery_filtered[,c("SNP", "P")]
write.table(ISO_SCORE, "ISO_SCORE.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
write.table(ISO_P, "ISO_P.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
...
setwd("/home/nodotea/PATRICIA/Objetivos/Calculos_PRS")
system2("./plink", args=c("--bfile EUGEI_WP2_Case_control_noMHC_filtered_CLUMPED", "--score ISO_SCORE.txt", "--q-score-file ISO_P.txt", "--q-score-range Q_RANGES.txt", "--out ISO_PRS_SCORE"))
...
setwd("/home/nodotea/PATRICIA/Objetivos/Calculos_PRS")
system2("./plink", args=c("--bfile EUGEI_WP2_Case_control_noMHC_new_filtered", "--indep-pairwise 500 1 0.1", "--out PCA_PRUNED"))
system2("./plink", args=c("--bfile EUGEI_WP2_Case_control_noMHC_new_filtered", "--extract PCA_PRUNED.prune.in", "--make-bed", "--out EUGEI_WP2_Case_control_noMHC_filtered_PRUNED"))
system2("./plink", args=c("--bfile EUGEI_WP2_Case_control_noMHC_filtered_PRUNED", "--pca", "--out PCA"))
...
familyInfo <- read.table("EUGEI_WP2_Case_control_noMHC_new_filtered.fam", header=FALSE)
...
age       <- read.table("AGE_WP2.txt", header=T, stringsAsFactors = FALSE)
covars    <- merge(covars, age, by = c("IID", "FID"))
country   <- read.table("COUNTRY_WP2.txt", header=T, stringsAsFactors = TRUE)
covars    <- merge(covars, country, by = c("IID", "FID"))
write.table(covars, "covariables_ISO.txt", row.names = FALSE, col.names = TRUE, dec = ".")

covars <- read.table("covariables_ISO.txt", header = TRUE, stringsAsFactors = FALSE)
covars_filter <- covars[covars$dcode %in% c(1, 2),] # Los pheno -9 se quitan. 
variables_ISO <- covars_filter

for (j in 1:12)  # Pongo 12 porque he generado 12 PRS para los 12 umbrales del archivo Q_RANGES
	{
  	fin <- read.table(paste0("ISO_PRS_SCORE.S",j,".profile"), header = TRUE, stringsAsFactors =  FALSE)
  	score <- data.frame(FID= fin$FID, IID= fin$IID, fin$SCORE)
  	colnames(score)[3] <- paste0("SCORE_S",j)
  	variables_ISO <- merge(variables_ISO, score, by = c("IID", "FID"))
	}

write.table(variables_ISO, "ISO.txt", dec= ".", row.names = FALSE)  #### GUARDAR A RESULTADOS ####
...
pheno <- as.numeric(variables_ISO$dcode)
PC1  <- variables_ISO$C1
PC2  <- variables_ISO$C2
PC3  <- variables_ISO$C3
PC4  <- variables_ISO$C4
PC5  <- variables_ISO$C5
PC6  <- variables_ISO$C6
PC7  <- variables_ISO$C7
PC8  <- variables_ISO$C8
PC9  <- variables_ISO$C9
PC10 <- variables_ISO$C10
SEX   <- variables_ISO$scode
AGE   <- variables_ISO$AGE
COUNTRY   <- variables_ISO$COUNTRY
...
for (j in 1:12) 
	{
  	score <- variables_ISO[,grep(paste0("^SCORE_S",j,"$"), colnames(variables_ISO))]  # tenemos que ponerle esta sintaxis para que coja score1 y no score10,11,12 a la vez
...
data_ISO_lrm = dataRanges # Mejor umbral S9 P < 0.1

write.table(data_ISO_lrm, "data_ISO_lrm.txt", dec= ",", row.names = FALSE)  #### GUARDAR A RESULTADOS ####


# Le añadimos el valor de P de la comparación inicial mediante glm si bootstrap.
table2 <- read.table("data_ISO_lrm.txt", header=T, stringsAsFactors = FALSE, dec = ",")
...
write.table(TABLA_FINAL, "FINAL_data_ISO_lrm.txt", dec= ",", row.names = FALSE)
...
## Después hacemos una comparación de la probabilidad de ser un caso con ISO dentro de cada decilr respecto al medio (5º) o respecto al decil más bajo (1º)
...
ISO <- read.table('ISO.txt', header=TRUE, stringsAsFactors = FALSE)  
ISO$decile <- ntile(ISO$SCORE_S3, 4) ### Asignar el umbral más significativo
...
for(i in y)
	{
  	variables_ISO <- ISO[ which(ISO$decile == 5 | ISO$decile == i), ]
 	write.table(variables_ISO, paste0("data",i), dec= ",", row.names = FALSE)
	}
...
for(i in y)
	{
  	variables_ISO <- read.table(paste0("data",i), header = TRUE, stringsAsFactors = FALSE, dec = ",")
  	variables_ISO$comparison <- ifelse(variables_ISO$decile == 5, 0, 1)
	...
	AGE   <- variables_ISO$AGE
	COUNTRY <- variables_ISO$COUNTRY
 	SCORE <- variables_ISO$comparison
...
write.table(data_decils_lgm, "data_ISO_lrm_DECIL_VS_5th.txt", dec= ",", row.names = FALSE)  #### GUARDAR A RESULTADOS ####
...
for(i in y)
	{
  	variables_ISO <- ISO[ which(ISO$decile == 1 | ISO$decile == i), ]
 	write.table(variables_ISO, paste0("data",i), dec= ",", row.names = FALSE)
	}
...
for(i in y)
	{
  	variables_ISO <- read.table(paste0("data",i), header = TRUE, stringsAsFactors = FALSE, dec = ",")
  	variables_ISO$comparison <- ifelse(variables_ISO$decile == 1, 0, 1)
...
data_decils_lgm = dataRanges
write.table(data_decils_lgm, "data_ISO_lrm_DECIL_VS_1st.txt", dec= ",", row.names = FALSE)   #### GUARDAR A RESULTADOS ####
...
prs.result <- read.table("FINAL_data_ISO_lrm.txt", header=T, stringsAsFactors = FALSE, dec = ",")
...
table2=read.table("data_ISO_lrm_DECIL_VS_5th.txt", header = T, dec = ",", stringsAsFactors = FALSE)
...
table2=read.table("data_ISO_lrm_DECIL_VS_1st.txt", header = T, dec = ",", stringsAsFactors = FALSE)
table2$Decile <- sub("Decile_", "", table2$Decile)
table2$Decile <- factor(table2$Decile, levels = table2$Decile)
