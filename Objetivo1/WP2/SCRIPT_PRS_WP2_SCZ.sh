...
# ==============================================
# Description -
#   El script utiliza los datos de genotipado de una muestra objetivo (cohorte de casos - controles EUGEI) que se quiere analizar (dataTarget) y los summary statistics de GWAS de esquizofrenia (dataDiscovery).
#
# ==============================================
...
dataDiscovery <- read.table('PGC3_SCZ_wave3.european.autosome.public.v3.vcf.tsv', header=TRUE, stringsAsFactors = FALSE)  ## 6498363 variantes
...
temp_discovery1$BP<-paste(temp_discovery1$CHROM, temp_discovery1$POS, sep=":")
...
temp_discovery3<-dataDiscovery[,c("CHROM", "ID", "BP", "A1", "A2",  "BETA", "SE", "PVAL")]
...
dataDiscovery <- dataDiscovery[!duplicated(dataDiscovery$SNP), ] #6498363 variantes
...
dataTarget_filtered <- dataTarget[which(dataTarget$varID %in% dataDiscovery$SNP),]                 ## 5416618       variantes
dataDiscovery_filtered <- dataDiscovery[which(dataDiscovery$SNP %in% dataTarget_filtered$varID),]  ## 5416618        variantes
...
setwd("/home/nodotea/PATRICIA/Objetivos/Calculos_PRS")
system2("./plink", args=c("--bfile EUGEI_WP2_Case_control_noMHC_filtered_CLUMPED", "--score SCZ_SCORE.txt", "--q-score-file SCZ_P.txt", "--q-score-range Q_RANGES.txt", "--out SCZ_PRS_SCORE"))
...
write.table(variables_SCZ, "SCZ.txt", dec= ".", row.names = FALSE)  #### GUARDAR A RESULTADOS ####
...
for(i in 1:n.simul) 
	...
	pseudocontrols = controls[sample(nrow(controls),size = 1561, replace = TRUE),]
...
SCZ$decile <- ntile(SCZ$SCORE_S12, 13) ### Asignar el umbral más significativo
