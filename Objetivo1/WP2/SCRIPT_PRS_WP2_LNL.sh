...
# ==============================================
# Description -
#   El script utiliza los datos de genotipado de una muestra objetivo (cohorte de casos - controles EUGEI) que se quiere analizar (dataTarget) y los summary statistics de GWAS de soledad (dataDiscovery).
#
# ==============================================
...
# summary statistics (LNL)
dataDiscovery <- read.table('LNL_ukbb_2018.txt', header=TRUE, stringsAsFactors = FALSE)  ## 7583883 variantes
...
LNL_SCORE <- dataDiscovery_filtered[,c("SNP", "A1", "BETA")]



LNL_P <- dataDiscovery_filtered[,c("SNP", "P")]
write.table(LNL_SCORE, "LNL_SCORE.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
write.table(LNL_P, "LNL_P.txt", sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE)
...
setwd("/home/nodotea/PATRICIA/Objetivos/Calculos_PRS")
system2("./plink", args=c("--bfile EUGEI_WP2_Case_control_noMHC_filtered_CLUMPED", "--score LNL_SCORE.txt", "--q-score-file LNL_P.txt", "--q-score-range Q_RANGES.txt", "--out LNL_PRS_SCORE"))
...
write.table(covars, "covariables_LNL.txt", row.names = FALSE, col.names = TRUE, dec = ".")

covars <- read.table("covariables_LNL.txt", header = TRUE, stringsAsFactors = FALSE)
covars_filter <- covars[covars$dcode %in% c(1, 2),] # Los pheno -9 se quitan. 
variables_LNL <- covars_filter

for (j in 1:12)  # Pongo 12 porque he generado 12 PRS para los 12 umbrales del archivo Q_RANGES
	{
  	fin <- read.table(paste0("LNL_PRS_SCORE.S",j,".profile"), header = TRUE, stringsAsFactors =  FALSE)
  	score <- data.frame(FID= fin$FID, IID= fin$IID, fin$SCORE)
  	colnames(score)[3] <- paste0("SCORE_S",j)
  	variables_LNL <- merge(variables_LNL, score, by = c("IID", "FID"))
	}

write.table(variables_LNL, "LNL.txt", dec= ".", row.names = FALSE)  #### GUARDAR A RESULTADOS ####
...
library(rms)
library(pROC)
variables_LNL <- read.table('LNL.txt', header=TRUE, stringsAsFactors = FALSE)  

# Definimos variables de regresión logística. Metemos las 10 primeras PCs, sexo y edad, además de PRS.
pheno <- as.numeric(variables_LNL$dcode)
PC1  <- variables_LNL$C1
PC2  <- variables_LNL$C2
PC3  <- variables_LNL$C3
PC4  <- variables_LNL$C4
PC5  <- variables_LNL$C5
PC6  <- variables_LNL$C6
PC7  <- variables_LNL$C7
PC8  <- variables_LNL$C8
PC9  <- variables_LNL$C9
PC10 <- variables_LNL$C10
SEX   <- variables_LNL$scode
AGE   <- variables_LNL$AGE
COUNTRY   <- variables_LNL$COUNTRY
...
for (j in 1:12) 
	{
  	score <- variables_LNL[,grep(paste0("^SCORE_S",j,"$"), colnames(variables_LNL))]  # tenemos que ponerle esta sintaxis para que coja score1 y no score10,11,12 a la vez
...
data_LNL_lrm = dataRanges # Mejor umbral S9 P < 0.1

write.table(data_LNL_lrm, "data_LNL_lrm.txt", dec= ",", row.names = FALSE)  #### GUARDAR A RESULTADOS ####


# Le añadimos el valor de P de la comparación inicial mediante glm si bootstrap.
table2 <- read.table("data_LNL_lrm.txt", header=T, stringsAsFactors = FALSE, dec = ",")
...
write.table(TABLA_FINAL, "FINAL_data_LNL_lrm.txt", dec= ",", row.names = FALSE)
...
## Después hacemos una comparación de la probabilidad de ser un caso con LNL dentro de cada decilr respecto al medio (5º) o respecto al decil más bajo (1º)
...
LNL <- read.table('LNL.txt', header=TRUE, stringsAsFactors = FALSE)  
LNL$decile <- ntile(LNL$SCORE_S10, 11) ### Asignar el umbral más significativo
...
for(i in y)
	{
  	variables_LNL <- LNL[ which(LNL$decile == 5 | LNL$decile == i), ]
 	write.table(variables_LNL, paste0("data",i), dec= ",", row.names = FALSE)
	}
...
for(i in y)
	{
  	variables_LNL <- read.table(paste0("data",i), header = TRUE, stringsAsFactors = FALSE, dec = ",")
  	variables_LNL$comparison <- ifelse(variables_LNL$decile == 5, 0, 1)
	...
	AGE   <- variables_LNL$AGE
	COUNTRY <- variables_LNL$COUNTRY
 	SCORE <- variables_LNL$comparison
...
write.table(data_decils_lgm, "data_LNL_lrm_DECIL_VS_5th.txt", dec= ",", row.names = FALSE)  #### GUARDAR A RESULTADOS ####
...
for(i in y)
	{
  	variables_LNL <- LNL[ which(LNL$decile == 1 | LNL$decile == i), ]
 	write.table(variables_LNL, paste0("data",i), dec= ",", row.names = FALSE)
	}
...
for(i in y)
	{
  	variables_LNL <- read.table(paste0("data",i), header = TRUE, stringsAsFactors = FALSE, dec = ",")
  	variables_LNL$comparison <- ifelse(variables_LNL$decile == 1, 0, 1)
...
data_decils_lgm = dataRanges
write.table(data_decils_lgm, "data_LNL_lrm_DECIL_VS_1st.txt", dec= ",", row.names = FALSE)   #### GUARDAR A RESULTADOS ####
...
prs.result <- read.table("FINAL_data_LNL_lrm.txt", header=T, stringsAsFactors = FALSE, dec = ",")
...
table2=read.table("data_LNL_lrm_DECIL_VS_5th.txt", header = T, dec = ",", stringsAsFactors = FALSE)
...
table2=read.table("data_LNL_lrm_DECIL_VS_1st.txt", header = T, dec = ",", stringsAsFactors = FALSE)
table2$Decile <- sub("Decile_", "", table2$Decile)
table2$Decile <- factor(table2$Decile, levels = table2$Decile)
