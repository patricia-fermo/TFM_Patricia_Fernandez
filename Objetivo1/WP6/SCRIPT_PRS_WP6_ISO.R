...
# ==============================================
# Description -
#   El script utiliza los datos de genotipado de una muestra objetivo (cohorte de casos - controles EUGEI) que se quiere analizar (dataTarget) y los summary statistics de GWAS de aislamiento social (dataDiscovery).
#
# ==============================================
...
dataDiscovery <- read.table('1031.gwas.imputed_v3.both_sexes.tsv', header=TRUE, stringsAsFactors = FALSE)  ## 13791468 variantes
...
dataTarget_filtered <- dataTarget[which(dataTarget$varID %in% dataDiscovery$SNP),]                 ## 6880783       variantes
dataDiscovery_filtered <- dataDiscovery[which(dataDiscovery$SNP %in% dataTarget_filtered$varID),]  ## 6880783       variantes
...
setwd("/home/nodotea/PATRICIA/Objetivos/Calculos_PRS")
system2("./plink", args=c("--bfile EUGEI_WP6_Case_control_noMHC_filtered_CLUMPED", "--score ISO_SCORE.txt", "--q-score-file ISO_P.txt", "--q-score-range Q_RANGES.txt", "--out ISO_PRS_SCORE"))
...
ISO$decile <- ntile(ISO$SCORE_S4, 5) ### Asignar el umbral más significativo
