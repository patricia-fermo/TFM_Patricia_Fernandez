#Script para análisis gen-ambiente de esquizofrenia (WP6), obtenido con Genomic SEM, con variables ambientales.
...
variables_ISO <- read.table('SCZ_only.txt', header=TRUE, stringsAsFactors = FALSE) 
variables_ISO <- variables_ISO %>% select(-c(SCORE_S1, SCORE_S2, SCORE_S3, SCORE_S4, SCORE_S5, SCORE_S6, SCORE_S7, SCORE_S8, SCORE_S9, SCORE_S10, SCORE_S12))
...
write.table(data, "SCZ_only_GxE.txt", sep="\t", quote=FALSE, row.names=FALSE)  #### GUARDAR A RESULTADOS ####
...
data <- read.table('SCZ_only_GxE.txt', header=TRUE, stringsAsFactors = FALSE) 
control_data = subset(data, dcode = 1)
cutoff = quantile(control_data$SCORE_S11, 0.75)
data$PGS = ifelse(data$SCORE_S11 < cutoff, 0, 1)
...
write.table(dataRanges, "RERI_results_SCZ_only.txt", sep="\t", quote=FALSE, row.names=FALSE)  #### GUARDAR A RESULTADOS ####
...
write.table(dataRanges, "RERI_results_PERMS_BOOTSTRAP_SCZ_only.txt", sep="\t", quote=FALSE, row.names=FALSE)  #### GUARDAR A RESULTADOS ####
...
table <- read.table("RERI_results_PERMS_BOOTSTRAP_SCZ_only.txt", sep="\t", header=T, stringsAsFactors = FALSE)
table_original <- read.table("RERI_results_SCZ_only.txt", sep="\t", header=T, stringsAsFactors = FALSE)
...
write.table(datos_nuevos, "RERI_results_CI_P_BOOTSTRAP_SCZ_only.txt", sep="\t", quote=FALSE, row.names=FALSE, dec = ",")  #### GUARDAR A RESULTADOS ####
